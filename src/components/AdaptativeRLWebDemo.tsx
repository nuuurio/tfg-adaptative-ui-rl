...
import {buildCombinedChartData, createInitialMultiUserChart} from "@/rl/charts";
import {contentLabel} from "@/utils/ui";

export default function AdaptiveRLWebDemo() {
    const [selectedUser,
        setSelectedUser] = useState < UserType > ("explorador");
    const [epsilon,
        setEpsilon] = useState(0.3);
    const [stepCount,
        setStepCount] = useState(0);
    const [qTable,
        setQTable] = useState < QTable > (loadInitialQ);
    const [lastDecision,
        setLastDecision] = useState < Decision | null > (null);
    const [history,
        setHistory] = useState < Decision[] > ([]);
    const [chartData,
        setChartData] = useState < {
        step: number;
        reward: number
    }[] > ([]);
    const [multiUserChart,
        setMultiUserChart] = useState(createInitialMultiUserChart);
    const [baselineChartData,
        setBaselineChartData] = useState < {
        step: number;
        reward: number
    }[] > ([]);
    ...
    const averageReward = chartData.length > 0
        ? (chartData.reduce((acc, val) => acc + val.reward, 0) / chartData.length).toFixed(2)
        : "0";
    ...
    const runOneTrainingStep = () => {
        ...
        const result = simulateUserResponse(selectedUser, action, {
            historyLength: history.filter((item) => item.userType === selectedUser).length,
            lastAction: history.find((item) => item.userType === selectedUser)
                ?.action ?? null
        });
    ...
    const trainMany = (steps = 50) => {
        ...
        const log : Decision[] = [];
        const nextMultiUserChart = JSON.parse(JSON.stringify(multiUserChart))as Record < UserType, {step: number;
                reward: number}[] >;
        ...
        for (let i = 0; i < steps; i++) {
            ...
            const {action, strategy} = chooseAction(nextQ[selectedUser], dynamicEpsilon);

            const result = simulateUserResponse(selectedUser, action, {
                historyLength: history.filter((item) => item.userType === selectedUser).length + i,
                lastAction: i === 0
                    ? history.find((item) => item.userType === selectedUser)
                        ?.action ?? null
                        : log[0]
                            ?.action ?? null
            });
    ...
    const trainAllUsers = (stepsPerUser = 20) => {
        ...
        userTypes.forEach((userType) => {
            for (let i = 0; i < stepsPerUser; i++) {
                ...
                const result = simulateUserResponse(userType, action, {
                    ...
                    lastAction: allHistory.find((item) => item.userType === userType)
                        ?.action ?? null
                });

                nextQ[userType][action] = updateQValue(nextQ[userType][action], result.reward);
        ...
        setMultiUserChart(nextMultiUserChart);
        setHistory((prev) => [
            ...allHistory.slice(0, 8),
            ...prev
        ].slice(0, 8));

        if (allHistory.length > 0) 
            setLastDecision(allHistory[0]);
        };
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="rounded-2xl border p-3 text-sm">
                                ...
                                <div className="mt-2 flex gap-2">
                                    <Button
                                        variant="outline"
                                        onClick={() => setEpsilon((prev) => Math.max(0, prev - 0.05))}>
                                        Baixar epsilon
                                    </Button>
                ...
                <div className="grid gap-6 lg:grid-cols-2">
                                ...
                                ? (
                                    <div className="space-y-2 text-sm">
                                        ...
                                        <p>
                                            ...
                                            {lastDecision.sessionStage}</p>
                                        <p>
                                            <strong>Acció repetida:</strong>
                                            {lastDecision.wasRepeated
                                                ? "sí"
                                                : "no"}</p>
                                        <p>
                                            <strong>Probabilitat de clic:</strong>
                                            {lastDecision.clickProbability}</p>
                                        <p>
                    ...
                    <Card className="rounded-2xl shadow-sm">
                        ...
                        <CardContent className="space-y-2 text-sm">
                            {userTypes.map((user) => {
                                const bestAction = actions.reduce((best, current) => qTable[user][current] > qTable[user][best]
                                    ? current
                                    : best);
